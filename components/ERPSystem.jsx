    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <header className="bg-gradient-to-r from-slate-800 to-slate-900 text-white px-6 py-4 ml-16">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm text-slate-400">{new Date().toLocaleDateString('en-GB', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
          </div>
        </div>
      </header>
          ...
          <div className="grid grid-cols-2 gap-6">
                ...
                {workOrders.map(wo => (
                  <div key={wo.id} className="p-4 hover:bg-gray-50">
                    ...
                    <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                      <div 
                        className={`h-full rounded-full ${wo.progress === 100 ? 'bg-green-500' : 'bg-blue-500'}`}
            ...
            <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
              <div className="p-4 border-b bg-amber-50 flex items-center justify-between">
                ...
                <span className="text-xs bg-red-500 text-white px-2 py-1 rounded-full">4 new</span>
              </div>
              <div className="divide-y">
                {alerts.map((a, i) => (
                  <div key={i} className={`p-4 flex items-center gap-3 hover:bg-gray-50 ${a.type === 'danger' ? 'bg-red-50/50' : ''}`}>
                    ...
                    {a.type === 'danger' && <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse" />}
                  </div>
                ))}
              </div>
            </div>
